...
module.exports = class HelloCommand extends SlashCommand {
    constructor(creator) {
        super(creator, {
            name: 'git',
            description: 'Kullanıcının yanına gitmek için izin alır',
            options: [
                {
    ...
    async run(ctx) {
        ...
        await ctx.send(`Sevgili ${mentioned}, <@${ctx.member.user.id}> kanalına katılmak istiyor. Eğer gelmesini istiyorsan bu kanala \`onayla @etiket/kullanıcıID\` yazman yeterli.`);
        ...
        collector.on("collect", async (message) => {
            if (message.content === `onay <@${mentioned.user.id}>` || message.content !== `onay ${mentioned.user.id}`) {
                ...
                await guild.members.cache.get(ctx.member.user.id).voice.setChannel(mentioned.voice.channel.id);
                await channel.send("Kullanıcı başarıyla kanalınıza taşındı.");
            }
        });
