...
class RolHobi extends ButtonCommand {
    constructor(client) {
        super(client, {
            name: "rol_hobi",
            cooldown: 10000
    ...
    async run(client, interaction, data) {
        ...
        if (interaction.values.includes("hobi_clear") && interaction.values.length === 1) {
            return await mentioned.roles.remove(Object.keys(data.roles).filter(key => key.startsWith("hobi_")).map(key => data.roles[key]));
        } else if (interaction.values.includes("hobi_clear")) {
        ...
        await mentioned.roles.remove(Object.keys(data.roles).filter(key => key.startsWith("hobi_")).map(key => data.roles[key]));
