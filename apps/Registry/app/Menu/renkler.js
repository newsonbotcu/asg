...
class RolSeçim extends Component {
    ...
    async run(ctx) {
        ...
        if (!ctx.data.data.values) return await mentioned.roles.remove(Object.keys(roles.value()).filter(key => key.startsWith("renk_")).map(key => roles.get(key).value()));
        const roleIDs = ctx.data.data.values.map(v => roles.get(v).value());
        const rolArray = roleIDs.map(rID => guild.roles.cache.get(rID));
        await mentioned.roles.remove(Object.keys(roles.value()).filter(key => key.startsWith("renk_")).map(key => roles.get(key).value()));
