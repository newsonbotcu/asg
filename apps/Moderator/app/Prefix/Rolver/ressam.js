...
class Painter extends Command {
    ...
    async run(client, message, args) {
        ...
        if (mentioned.roles.cache.has(roles.get("role_ressam").value())) {
            await mentioned.roles.remove(roles.get("role_ressam").value());
            await message.react(emojis.get("ok").value().split(':')[2].replace('>', ''));
        } else {
            await mentioned.roles.add(roles.get("role_ressam").value());
            await message.react(emojis.get("ok").value().split(':')[2].replace('>', ''));
            emoji = "error"
            durum = "Alındı";
        }
        ...
        ${message.member} (\`${message.member.user.id}\`) adlı yetkili, ${mentioned} (\`${mentioned.user.id}\`) üyesin${durum === "Verildi" ? "e" : "den"} <@&${roles.get("role_ressam").value()}> rolü ${durum.toLowerCase()}.
