...
class vMute extends Command {
    ...
    async run(client, message, args) {
        ...
        if (!mentioned) {
            ...
            return message.reply(new Discord.MessageEmbed().setDescription(`${data.emojis["kullaniciyok"]} Kullanıcı bulunamadı!`).setColor('#2f3136')).then(msg => msg.delete({ timeout: 1000 }));
        }
        if (!sebep) {
            await message.react(data.emojis["error"].split(':')[2].replace('>', ''));
            return message.reply(new Discord.MessageEmbed().setColor('#2f3136').setDescription(`${data.emojis["soru"]} Bir sebep girmelisin`)).then(msg => msg.delete({ timeout: 1000 }));
        }
        if (message.member.roles.highest.rawPosition <= mentioned.roles.highest.rawPosition) {
            await message.react(data.emojis["error"].split(':')[2].replace('>', ''));
            return message.reply(new Discord.MessageEmbed().setColor('#2f3136').setDescription(`${data.emojis["missingPerms"]} Bunu yapmak için yeterli yetkiye sahip değilsin`)).then(msg => msg.delete({ timeout: 1000 }));
        }
        if (!sayi(args[1])) {
            await message.react(data.emojis["error"].split(':')[2].replace('>', ''));
            return message.reply(new Discord.MessageEmbed().setColor('#2f3136').setDescription(`${data.emojis["sayifalan"]} Geçerli bir dakika girmelisin`)).then(msg => msg.delete({ timeout: 1000 }));
        }
