...
class Kur extends Command {
    ...
    async run(client, message, args, data) {
        ...
        if (!efct) return message.react(data.emojis["error"].split(':')[2].replace('>', ''));

        let typo;
        if (args[2] === "ekle") typo = "create";
        if (args[2] === "sil") typo = "delete";
        if (args[2] === "yenile") typo = "update";
        if (!typo) return message.react(data.emojis["error"].split(':')[2].replace('>', ''));
        if (!sayi(args[3])) return message.react(data.emojis["error"].split(':')[2].replace('>', ''));
        if (!sayi(args[4])) return message.react(data.emojis["error"].split(':')[2].replace('>', ''));
        if (args[4] > 5) return message.react(data.emojis["error"].split(':')[2].replace('>', ''));
